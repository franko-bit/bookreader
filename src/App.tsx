...
export default function App() {
  ...
  async function runTool(kind: 'translation' | 'summary', style?: 'short' | 'bullets') {
    ...
    try {
      if (kind === 'translation') { const response = await ai.translate(focusText, language); setResult({ title: `${language} translation`, body: response.translation, kind }) }
  ...
  return <div className={dark ? 'app-shell' : 'app-shell bg-slate-100 text-slate-900'}>
    ...
    <main className="grid min-h-[calc(100vh-73px)] grid-cols-1 gap-4 p-4 xl:grid-cols-[220px_minmax(0,1fr)_340px]">
      ...
      <aside className="space-y-4"><section className="panel p-4"><h2 className="mb-3 text-sm font-semibold">Selected text tools</h2><div className="grid grid-cols-2 gap-2"><button className="toolbar-button justify-center" onClick={() => runTool('summary', 'short')}>Summary</button><button className="toolbar-button justify-center" onClick={() => runTool('summary', 'bullets')}>Key points</button><select aria-label="Translation language" value={language} onChange={(event) => setLanguage(event.target.value)} className="rounded border border-slate-600 bg-slate-800 px-2 text-sm"><option>French</option><option>Kinyarwanda</option><option>English</option></select><button className="toolbar-button justify-center" onClick={() => runTool('translation')}>Translate</button></div><button className="toolbar-button mt-2 w-full justify-center" onClick={speak}><Volume2 size={16} /> Read aloud</button>{result && <div className="mt-4 rounded-lg bg-slate-800 p-3"><p className="m-0 text-sm font-semibold">{result.title}</p><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-300">{result.body}</p></div>}</section>
